...
// 获取未填写完毕的分组的下标
function getUnFilledPartInds(sections: SectionFormItem[], form: any) {
...
export default forwardRef((props: Props, ref: any) => {
  ...
  useEffect(() => {
    getUnFilledIndexs?.(getUnFilledPartInds(sections, form));
  ...
  const debounceHandleFinish = useCallback(
    debounce((args: CommonObj) => handleFinish(args, allFields, props, { setLoading, closePopup, fetchSuccess, fetchFail })),
    []
  );
  // 防抖处理 onValuesChange 事件
  const handelValuesChange = useCallback(
    debounce((vals: CommonObj, allVals: CommonObj) => {
      getUnFilledIndexs?.(getUnFilledPartInds(sections, form));
